...
grid = np.array([list(x.strip()) for x in open('test.txt')], int)
...
for _ in range(4):
    for x,y in np.ndindex(grid.shape):   
            ...
            print(f"{x} {y} : {lower}, {[i+1 for i,t in enumerate(lower) if ~t]}, {next((i+1 for i,t in enumerate(lower) if ~t), len(lower))}")
            part1[x,y] |= all(lower)
